...
def plot_images(image, shape=None, num_row=2, num_col=5, scale=1, row_title=None, fname=None, save=False):
    ...
    fig, axes = plt.subplots(num_row, num_col, figsize=(
        1.5*scale*num_col, 2*scale*num_row))
...
def plot_visualization(image, shape=None, num_row=3, num_col=5, scale=1, row_title=None, fname=None, save=False):
    ...
    fig, big_axes = plt.subplots(num_row, 1, figsize=(
        1.5*scale*num_col, 2*scale*num_row), sharey=True)
...
def normalize(x_train, y_train, x_test, y_test):
    x_train = x_train / onp.sqrt(onp.reshape(onp.einsum('ij,ij->i',
                                 x_train, x_train), (64, 1))) * onp.sqrt(x_train.shape[-1])
    y_train = y_train - onp.mean(y_train, axis=0, keepdims=True)
    x_test = x_test / onp.sqrt(onp.reshape(onp.einsum('ij,ij->i',
                               x_test, x_test), (32, 1))) * onp.sqrt(x_test.shape[-1])
...
def get_dataset(name, data_dir, n_train=None, n_test=None, permute_train=False, flatten=False, normalize=False):
